...
// css(box, 'display', 'none'); var iw = css(box, 'width');注意调用的attr应该用引号括起来，传进去的是一个属性常量值
/*
...
//正则网址大全：https://www.cnblogs.com/fozero/p/7868687.html
//封装深度拷贝
...
function ajax(opt) {
    ...
    xhr.onreadystatechange = () => {
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                //成功  
                defaultOpt.success(xhr.responseText);//实参。responseText：保存服务器返回的数据（从服务器返回的数据是“字符串”）。
            } else {
                //失败
                if (defaultOpt.error) {
