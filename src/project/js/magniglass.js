function magniglass(opt) {
    ...
    smaller.firstElementChild.classList.add('current');
    //点击哪个哪个高亮
    var smallerlis = document.querySelectorAll('.smaller li');
    // console.log(smallerlis);
    for (var i = 0; i < smallerlis.length; i++) {
        smallerlis.index = i;
        smallerlis[i].onclick = function () {
            for (var j = 0; j < imgArr.length; j++) {
                smallerlis[j].classList.remove('current');
            }
    ...
    main.onmousemove = function (ev) {
        ...
        var top = ev.pageY - wrap.offsetTop - mask.offsetHeight / 2 - 200;
        if (left <= 0) {//临界值设置
            left = 0;
